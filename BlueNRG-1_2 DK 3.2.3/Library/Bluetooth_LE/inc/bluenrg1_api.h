 ...
 *        Value length returned: 1 byte * 0x2E: The static random address
 *        overwritten by application (used instead of the one stored in NVM). *
 *        0x80: The static random address stored in NVM. Value length returned:
 *        6 bytes (read-only)
 *        Values:
 *        - 0x00: CONFIG_DATA_PUBADDR_OFFSET
 *        - 0x08: CONFIG_DATA_ER_OFFSET
 ...
 * @param En_High_Power Enable High Power mode.  High power (0x01) is the
 *        default.
 *        Values:
 *        - 0x00: Normal Power
 *        - 0x01: High Power
 * @param PA_Level Power amplifier output level. Output power is indicative and
 *        it depends on the PCB layout and associated components
 *        Values:
 *        - 0: -14 dBm (High Power)
 *        - 1: -11 dBm (High Power)
 *        - 2: -8 dBm (High Power)
 ...
 *             - 0x03: Scanning - 0x04: Reserved - 0x05: Connected as master -
 ...
 *        application in selecting slot timings when operating in multi-link
 *        scenarios.
