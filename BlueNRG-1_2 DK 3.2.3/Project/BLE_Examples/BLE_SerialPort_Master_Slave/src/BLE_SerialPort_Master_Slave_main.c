...
* Version            : 1.1.0
* Date               : 24-August-2021
* Description        : Code demostrating the BLE Serial Port Master and Slave application (single application for both devices)
********************************************************************************
...
/**
 * @file BLE_SerialPort_Master_Slave_main.c
 * @brief This is a Serial Port demo that shows how to implement a simple 2-way communication between only two BlueNRG-1,2 devices (single application for both devices).
 * 
...
* \section KEIL_project KEIL project
  ...
  -# Open the KEIL project
     <tt> C:\Users\{username}\ST\BlueNRG-1_2 DK x.x.x\\Project\\BLE_Examples\\BLE_SerialPort_Master_Slave\\MDK-ARM\\BlueNRG-1\\BLE_SerialPort_Master_Slave.uvprojx </tt> or
     <tt> C:\Users\{username}\ST\BlueNRG-1_2 DK x.x.x\\Project\\BLE_Examples\\BLE_SerialPort_Master_Slave\\MDK-ARM\\BlueNRG-2\\BLE_SerialPort_Master_Slave.uvprojx </tt>
...
* \section IAR_project IAR project
  ...
  -# Open the IAR project
     <tt> C:\Users\{username}\ST\BlueNRG-1_2 DK x.x.x\\Project\\BLE_Examples\\BLE_SerialPort_Master_Slave\\EWARM\\BlueNRG-1\\BLE_SerialPort_Master_Slave.eww </tt> or
     <tt> C:\Users\{username}\ST\BlueNRG-1_2 DK x.x.x\\Project\\BLE_Examples\\BLE_SerialPort_Master_Slave\\EWARM\\BlueNRG-2\\BLE_SerialPort_Master_Slave.eww </tt>
  -# Select desired configuration to build
  -# Select Project->Rebuild All. This will recompile and link the entire application
  -# To download the binary image, please connect a SWD HW programmer in your board (if available).
...
* \section Usage Usage
The Serial Port Master and Slave application configures a BlueNRG-1,2 device as Central & Peripheral using the aci_gap_init(GAP_CENTRAL_ROLE|GAP_PERIPHERAL_ROLE, ...). Then it performs the following steps:
 ...
 - If discovery procedure time expires without finding another Serial Port Master and Slave device,  the device enters in discovery mode, waiting for another Serial Port Master and Slave device to discover and connect to it. 
 - If discovery mode time expires without any connection established, the device starts again a discovery procedure for another Serial Port Master and Slave device.
 - When a connection is established, the device role are defined (Client or Server) and the Serial Port communication channel can be used.  
 ...
 * BlueNRG-1,22 Serial Port Master and Slave demo \see BLE_SerialPort_Master_Slave_main.c for documentation.
 *
 *@{
 */
...
int main(void)
{
  ...
  printf("BlueNRG-1,2 BLE Serial Port Master & Slave Application (version: %s)\r\n",BLE_NEW_SERIALPORT_VERSION_STRING); 
