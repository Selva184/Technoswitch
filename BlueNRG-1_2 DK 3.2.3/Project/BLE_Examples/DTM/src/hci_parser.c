...
#define HCI_PACKET_SIZE USER_HCI_PCKT_SIZE//259 // Maximum size of HCI packets are 255 bytes + the HCI header (3 bytes) + 1 byte for transport layer.
...
hci_state hci_input(uint8_t *buff, uint16_t len)
{
  ...
  while(hci_pckt_len < HCI_PACKET_SIZE && i++ < len){
    ...
    if(state == WAITING_TYPE){
      ...
      state = WAITING_PARAM_LEN_1;
    }
		else if(state == WAITING_PARAM_LEN_1){
			payload_len = byte;//((byte>0)?(byte-1u):byte);
      state = WAITING_PARAM_LEN_2;
    }
		else if(state == WAITING_PARAM_LEN_2){
			payload_len += byte<<8;
