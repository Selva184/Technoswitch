...
#include "cmd.h"
#include "ble.h"
#define HCI_MAX_PAYLOAD_SIZE CUSTOM_MAX_PAYLOAD_SIZE//256
/* Private typedef -----------------------------------------------------------*/
typedef SleepModes (*DTM_SleepMode_Check_Type)(SleepModes sleepMode);

/* Private define ------------------------------------------------------------*/
#define EVENT_BUFFER_SIZE    CUSTOM_EVT_BUFFER_SIZE//1024
#define COMMAND_BUFFER_SIZE  CUSTOM_CMD_BUFF_SIZE//(265)  /* 258 (HCI header +  payload) + 2 (alignment) + 4 (fifo item length) + 1 spare */
#define FIFO_VAR_LEN_ITEM_MAX_SIZE USER_CUSTOM_ATT_MTU_SIZE+4 //512+4//512+3//258
...
{
	#if EN_512_ATT_MTU
  uint8_t buffer[HCI_MAX_PAYLOAD_SIZE], buffer_out[HCI_MAX_PAYLOAD_SIZE];
	#else
	uint8_t buffer[HCI_MAX_PAYLOAD_SIZE], buffer_out[HCI_MAX_PAYLOAD_SIZE];
  ...
  if ((fifo_size(&command_fifo) > 0) && (!reset_pending)) {
    ...
    /* Set user events to temporary queue */    
		...
		#if EN_512_ATT_MTU
		len=process_command(opcode, buffer + 4, size - 4, buffer_out, HCI_MAX_PAYLOAD_SIZE);//255);
		#else
