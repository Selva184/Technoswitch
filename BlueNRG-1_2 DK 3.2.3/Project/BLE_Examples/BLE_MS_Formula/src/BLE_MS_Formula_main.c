...
* \section KEIL_project KEIL project
  ...
  -# Open the KEIL project
     <tt> C:\Users\{username}\ST\BlueNRG-1_2 DK x.x.x\\Project\\BLE_Examples\\BLE_MS_Formula\\MDK-ARM\\BlueNRG-1\\BLE_MS_Formula.uvprojx </tt> or
...
* \section IAR_project IAR project
  ...
  -# Open the IAR project
     <tt> C:\Users\{username}\ST\BlueNRG-1_2 DK x.x.x\\Project\\BLE_Examples\\BLE_MS_Formula\\EWARM\\BlueNRG-1\\BLE_MS_Formula.eww </tt> or
...
1) Configure Master_Slave device 
 - Download the Project\\BLE_Examples\\BLE_MS_Formula\\EWARM\\[BlueNRG-1/BlueNRG-2]\\BLE_MS_Formula_Master_Slave\\Exe\\BLE_MS_Formula_Master_Slave.hex, respectively, on the selected BlueNRG-1 or BlueNRG-2 STEVAL kit.
...
2) Configure Num_Slaves GAP Peripheral devices Slave1, ... 

 - Download the Project\\BLE_Examples\\BLE_MS_Formula\\EWARM\\[BlueNRG-1/BlueNRG-2]\\BLE_MS_Formula_Slave\\Exe\\BLE_MS_Formula_Slave.hex, respectively, on the selected BlueNRG-1 or BlueNRG-2 STEVAL kit.
 - Open a hyper terminal with the described settings
 - Reset the selected BlueNRG-1 or BlueNRG-2 STEVAL kit: the selected device enters in discoverable mode. 
 - If a new slave is requested, make sure to change the related address on Slave.c code by properly defining the SLAVE_INDEX variable.

NOTE: Alternatevely user can configure slaves devices by running the Slaves.py script: 
 
 - <b>BlueNRG-MS platform configuration steps</b>:
 ...
 - <b> BlueNRG-1,2 platforms configuration steps</b>: 
	 ...
	 - BlueNRG-1 platform: download the "C:\\\Program Files (x86)\STMicroelectronics\\BlueNRG GUI x.x.x\\Firmware\\DTM\\BlueNRG1\\DTM_UART.hex or DTM_SPI.hex" file, using the BlueNRG Flasher utility available on  STSW-BNRGFLASHER SW package 
	 - BlueNRG-2 platform: download the "C:\\Program Files (x86)\\STMicroelectronics\\BlueNRG GUI x.x.x\\Firmware\\DTM\\BlueNRG1\\DTM_UART.hex or DTM_SPI.hex" file, using the BlueNRG Flasher utility available on  STSW-BNRGFLASHER SW package 
...
Once Master device finds advscan device it connects to it,  and it enables the characteristic notification. Notifications from Num_Slaves devices will be notified to Master device through the Master_Slave device. 
 ...
 - Download the Project\\BLE_Examples\\BLE_MS_Formula\\EWARM\\[BlueNRG-1/BlueNRG-2]\\BLE_MS_Formula_Master\\Exe\\BLE_MS_Formula_Master.hex, respectively, on the selected BlueNRG-1 or BlueNRG-2 STEVAL kit.
 - Open a hyper terminal with the described settings
