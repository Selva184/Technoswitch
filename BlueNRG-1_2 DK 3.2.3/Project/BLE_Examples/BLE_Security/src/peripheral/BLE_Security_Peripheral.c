...
{
  tBleStatus ret;
  uint32_t random_number[2];
  uint32_t random_key; 
  
  /* get a random number from BlueNRG-1 */ 
  ret = hci_le_rand((uint8_t *)random_number);
  if(ret != BLE_STATUS_SUCCESS)
     PRINTF("hci_le_rand() call failed: 0x%02x\n", ret);
  
  random_key = random_number[0] % 1000000;
...
void APP_Tick(void)
{
  ...
  {
    PRINTF("** Peripheral Security Key (IO DISPLAY): %06d. INSERT IT on CENTRAL Device !\r\n", (int)Peripheral_Pass_Key);
    PRINTF("\r\n");
