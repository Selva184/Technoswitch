...
#define BLE_SERIAL_PORT_COMPLETE_LOCAL_NAME_SIZE 12 
...
void Reset_DiscoveryContext(void)
{
  ...
  discovery.device_state = BLE_INIT;
...
uint8_t Find_DeviceName(uint8_t data_length, uint8_t *data_value)
{
  ...
  while (index < data_length) {
    ...
    /* Check if field is complete local name and the lenght is the expected one for BLE Serial Port  */
    if (data_value[index+1] == AD_TYPE_COMPLETE_LOCAL_NAME) {
      /* check if found device name is the expected one: local_name */ 
      if (memcmp(&data_value[index+1], &local_name[0], BLE_SERIAL_PORT_COMPLETE_LOCAL_NAME_SIZE) == 0)
        return TRUE;
      else
        return FALSE;
    } else {
