/**
  ******************************************************************************
  * @file    UART/Polling/BlueNRG1_it.c 
...
{ 
  /* RX interrupt triggered by the RX FIFO threshold */
  if (UART_GetITStatus(UART_IT_RX) != RESET)
  {
    /* Clear the interrupt */
    UART_ClearITPendingBit(UART_IT_RX);
    
    /* Echo of received data and empties */
    while (UART_GetFlagStatus(UART_FLAG_RXFE) == RESET) {
      UART_SendData((UART_ReceiveData() & 0xFF));
    }
    
    /* Toggle LED2 if receive irq occurs on fifo level */
    SdkEvalLedToggle(LED2);
  }
    
  /* RX timeout interrupt */
  if (UART_GetITStatus(UART_IT_RT) != RESET)
  {
    /* Clear the interrupt */
    UART_ClearITPendingBit(UART_IT_RT);
    ...
    /* Toggle LED1 if receive timeout occurs */
