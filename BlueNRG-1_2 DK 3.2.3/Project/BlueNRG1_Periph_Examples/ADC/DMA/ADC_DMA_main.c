  ...
  while(1) {
    ...
    if(DMA_GetFlagStatus(DMA_FLAG_TC0)) {      
      ...
      for(uint16_t i=0;i<ADC_DMA_BUFFER_LEN;i++) {
        ...
        adc_value_avg_f += adc_value_f;
      }
      adc_value_avg_f /= ADC_DMA_BUFFER_LEN;
      
      /* Apply the compensation formula and convert to mV */
      adc_value_avg_f = ADC_CompensateOutputValue(adc_value_avg_f) *1000.0;
      printf("ADC value: %d.%03d mV\r\n", PRINT_INT(adc_value_avg_f), PRINT_FLOAT(adc_value_avg_f));
